...
def convert_waveform(
    ...
    try:
        ...
        return _convert_waveform_no_sox(
            waveform,
            sample_rate,
...
_TORCHAUDIO_WARNING_EMITTED = False


def _convert_waveform_no_sox(
    waveform: Union[np.ndarray, torch.Tensor],
    sample_rate: int,
    *,
    ...
    global _TORCHAUDIO_WARNING_EMITTED
    if not _TORCHAUDIO_WARNING_EMITTED:
        warnings.warn(
            "torchaudio.sox_effects is unavailable; falling back to a minimal numpy/torch implementation "
            "for waveform conversion. Consider installing torchaudio for higher-quality resampling.",
            RuntimeWarning,
            stacklevel=2,
        )
        _TORCHAUDIO_WARNING_EMITTED = True

    is_np_input = isinstance(waveform, np.ndarray)
    tensor_waveform = (
        torch.from_numpy(waveform).float() if is_np_input else waveform.float()
    )

    if to_mono and tensor_waveform.shape[0] > 1:
        tensor_waveform = tensor_waveform.mean(dim=0, keepdim=True)

    if normalize_volume:
        with torch.no_grad():
            peak = tensor_waveform.abs().max()
            if peak > 0:
                tensor_waveform = tensor_waveform / peak

    target_sample_rate = sample_rate if to_sample_rate is None else to_sample_rate
    if target_sample_rate != sample_rate:
        ratio = target_sample_rate / sample_rate
        length = tensor_waveform.shape[-1]
        target_length = max(int(round(length * ratio)), 1)
        tensor_waveform = tensor_waveform.unsqueeze(0)
        tensor_waveform = F.interpolate(
            tensor_waveform,
            size=target_length,
            mode="linear",
            align_corners=False,
        )
        tensor_waveform = tensor_waveform.squeeze(0)

    if is_np_input:
        return tensor_waveform.numpy(), target_sample_rate
    return tensor_waveform, target_sample_rate
